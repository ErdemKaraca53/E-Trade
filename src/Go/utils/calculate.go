...
// En verimli gönderim yolunu bulan fonksiyon
...
func CalculateProductStock(Order Order, db *sql.DB) ([][]Distance, []LocationsForCombination, []int, []CargoInfo, string) {
	...
	//Sepetteki ürün adedi kadar dönüyorum
...
// Depoların istenilen adrese mesafelerini bulan fonksiyon
...
// Kargo bilgilerini veritabanından çekme
...
// Stokların olduğu depoları bulan fonksiyon
...
// Stok durumuna göre mesafeleri filtreler.
...
// Mesafe dizisini yakınlığa göre filtreler
...
// Tüm kombinasyonları recursive şeklinde bulur.
...
// Tüm Depoların kapasitesini çeker
...
// Tüm kombinasyonların puanını hesapla
func CalculateAllCombinations(allCombinations [][]int, matrixDistances [][]Distance, CargoInfos []CargoInfo, Locations []Locations, productAmount int) BestCombination {
	...
	for i := 1; i < len(allCombinations); i++ {
		...
		// Gelen kombinasyondaki ürünlerin hangi kargodan çıkması gerektiğini bir matriste sınıflandırıyor
		for j, locID := range allCombinations[i] {
			sameArray[locID] = append(sameArray[locID], j)
		}
		fmt.Println(sameArray)

		// Kombinasyonun maliyeti ve depo kapasitesi hesaplanıyor
		...
		/*fmt.Println("Yol: ", totalWays)
		fmt.Println("maliyet: ", cost)
		fmt.Println("maliyetPuanı: ", capacityPoint)
		fmt.Println("Kapasite Puanı: ", costPoint)
		fmt.Println("Toplam Puan: ", totalPoint)*/
	...
	/*fmt.Println("Best Point:", BestOrder.Point)
	fmt.Println("Best Cost:", BestOrder.Cost)
	fmt.Println("Best Way:", BestOrder.Ways)
	fmt.Println("Best Combination:", bestCombination)
	for index, value := range BestOrder.Combination {
		fmt.Println("product:", index)
		fmt.Println("Best location:", value)
	}*/
	return BestOrder
}

// Kargoları istenilen lokasyona göre filtreler
...
// Kargoların maliyeti hesaplayıp en ucuzunu return eder.
...
// Allocator sonuçlarını order'a ekleme
